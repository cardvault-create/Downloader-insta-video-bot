...
class InstaDownloader:
    ...
    def _download_video(url, shortcode):
        try:
            ...
            formats_to_try = ['best', 'best[ext=mp4]', 'bestvideo+bestaudio']
            for fmt in formats_to_try:
                try:
                    ydl_opts = {
                        'quiet': True, 'no_warnings': True, 'ignoreerrors': True,
                        'outtmpl': os.path.join(DOWNLOAD_DIR, f'{shortcode}.%(ext)s'),
                        'format': fmt, 'cookiefile': 'cookies.txt',
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if thumbnail_url:
                    ...
                    return InstaDownloader._save_images([hd_url, thumbnail_url], shortcode)
        except:
            pass
        return {"success": False}
                        ...
                        if isinstance(val, dict):
                            for k2, v2 in val.items():
                                if isinstance(v2, dict) and v2.get('display_url'):
                                    return InstaDownloader._save_images([v2['display_url']], shortcode)
        except:
            pass
        return {"success": False}
    ...
    def extract_audio(video_path, custom_name=None):
        try:
            if custom_name:
                ...
                if not safe_name: safe_name = "Instagram_Audio"
