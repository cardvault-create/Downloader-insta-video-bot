...
# ═══════════════════════════
# 📝 TEXT (parse_mode=None for safety)
# ═══════════════════════════

def send_msg(update, text, **kwargs):
    """Safe message sending - no parse errors"""
    try:
...
CAPTION = (
    "Downloaded By - @Instagram_LinkToVideo_Bot\n"
    "\n"
...
async def bot_added_to_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.my_chat_member.new_chat_member.status in [ChatMemberStatus.MEMBER, ChatMemberStatus.ADMINISTRATOR]:
        ...
        await context.bot.send_message(chat.id, f"Hello {chat.title}!\n\nSend Instagram link to download.", reply_markup=kb)
...
async def list_emojis_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Emojis: {len(emojis)}")
...
async def list_stickers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Stickers: {len(stickers)}")
...
async def list_videos_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Videos: {len(vids)}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_video:
            ...
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Download Audio", callback_data=f"aud_{url}")]])
...
async def extract_and_send_audio(update, context, url, audio_name):
    ...
    try:
        ...
        else: await status_msg.edit_text(ar.get('error'))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("aud_"):
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("My Music", callback_data="def_audio")]])
        ...
        if url: await extract_and_send_audio_direct(query, context, url, "My Music")
